...
## structure search ####
npatlas_matches = read_tsv("data/QTOF/moldiscovery_npatlas/significant_matches_filtered.tsv")
npatlas_matches$prismMatch = mclapply(npatlas_matches$SMILES, function(smile) {
  system(
    sprintf(
      "obabel npdtools/prism_GB4-14_pb/prism_combined.fs -osmi -s\"%s\" -at0.65",
      smile
    ),
    intern = T,
    ignore.stderr = T
